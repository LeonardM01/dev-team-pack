...
Albus is the orchestrator - he does one research pass up front, scopes the blueprint to the smallest change that meets the acceptance criteria, then delegates implementation to Harry as one packet by default (split only above ~14 files or into parallel packets on disjoint file sets; the first packet opens with the ground check and a recon list; packets point at the brief, blueprint and `PROGRESS.md` rather than pasting them). The full build, test suite and lint run once at the end of implementation and once more after a review fix. Hermione then reviews the whole diff against the blueprint and requirements (max 2 rounds - full review, one fix packet, one re-check - blocker/should-fix/nit severity), then Ron updates docs. The run is tracked as a checklist in `PROGRESS.md` in the working directory; each agent ticks its own items, and a previous run's file is archived as `PROGRESS-<n>.md`.
