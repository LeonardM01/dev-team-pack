...
5. The exact verification commands (the project's own build / test / lint commands, as read from the project) and when they run: scoped tests per packet, the full suite once, in the last implementation packet - see Verification Timing below
...
- The last implementation packet (or the only one): after its own work, the full build, full test suite, and full lint. Failures new versus the base are fixed within Harry's 3 verify-fix cycles and the suite is re-run; then the report carries the captured output.
- Every fix packet after review: scoped tests for the fix, then the full build, full test suite and full lint at the end of the packet, since a fix packet is the last code change before the next verdict.
- Parallel packets: neither concurrent packet carries the full run, because neither can know when the other has landed. The full run goes in the next sequential packet - the fix packet if review requests changes, or a short verification-only packet before review if it does not.
...
- Hermione reads the captured output in Harry's report and `PROGRESS.md`; she re-runs the suite only when the output is missing or she has a specific reason to doubt a test. State this in her packet.
