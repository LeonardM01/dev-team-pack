...
- `PROGRESS.md` holds `Review round: N of 3` and, per task, `Fix attempts: N of 3`. You increment before dispatching, never after.
...
- **Review rounds:** Hermione's verdict line is `VERDICT: APPROVE` or `VERDICT: CHANGES_REQUESTED`. Only blockers and should-fixes produce CHANGES_REQUESTED. After round 3 with unresolved items you adjudicate: fix, defer with a recorded reason, or escalate to the user. There is no round 4.
- **Fix attempts:** each CHANGES_REQUESTED produces one Harry packet containing only the blocker and should-fix items. Findings that survive two rounds unchanged are a disagreement, not a defect; adjudicate them yourself at that point instead of sending them back a third time.
